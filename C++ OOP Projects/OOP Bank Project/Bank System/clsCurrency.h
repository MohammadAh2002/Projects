...
private:
    ...
    {
        vector<string> vCurrencyData = clsString::SplitStringInVector(Line, Seperator);
...
public:
    ...
    static   clsCurrency FindByCountry(string Country)
    {
        ...
        if (MyFile.is_open())
        {
            ...
            while (getline(MyFile, Line))
            {
                ...
                if (clsString::StringToUpper(Currency.GetCountry()) == Country)
                {
                    MyFile.close();
                    return Currency;
                }
    ...
    float ConvertToOtherCurrency(float Amount, clsCurrency CurrencyTo)
    {
        ...
        return (float)ConvertToUSD(Amount) * CurrencyTo.GetRate();
