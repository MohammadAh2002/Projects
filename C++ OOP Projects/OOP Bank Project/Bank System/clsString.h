...
public:
    ...
    static int GetLength(string S1)
    {
        return S1.length();
    };
    int GetLength()
    ...
    static string DcryptText(string text, short encryptionKey = 2)
    {
        for (int i = 1; i <= text.length(); i++)
        {
            text[i] = char((int)text[i] - encryptionKey);
    ...
    string DcryptText(short encryptionKey) {

        return DcryptText(_Value, encryptionKey);
    ...
    static string ReplaceWordInString(string S1, string StringToReplace, string sRepalceTo, bool MatchCase = true)
    {
    ...
    vector <string> SplitStringInVector() {

        return SplitStringInVector(_Value);
