...
/*
	...
	Build a Date Class to manage and manipulate date operations.
	...
	- SetYear(short Year)
	- GetYear()
	- DateFromStringToDate(string StringDate)
	- GetSystemDate()
	- NumberOfDaysInMonth()
	- NumberOfDaysInYear()
	- NumberOfHoursInYear()
	- NumberOfMinutesInYear()
	- NumberOfSecondsInYear()
	- NumberOfHoursInMonth()
	- NumberOfMinutesInMonth()
	- NumberOfSecondsInMonth()
	- IsLeapYear()
	- IsDate1BeforeDate2(clsDate Date2)
	- IsDate1AfterDate2(clsDate Date2)
	- IsDate1EqualsDate2(clsDate Date2)
	- IsLastDayInMonth()
	- IsLastMonthInYear()
	- IsEndOfTheWeek()
	- IsWeekEnd()
	- IsBusinessDay()
	- IsValidDate()
	- DayName(short DayNum)
	- DayName()
	- MonthName(short MonthNum)
	- MonthName()
	- DaysUntilTheEndOfTheWeek(clsDate Date)
	- DaysUntilTheEndOfTheWeek()
	- DaysUntilTheEndOfTheMonth(clsDate Date, bool IncludeCurrentDay = false)
	- DaysUntilTheEndOfTheMonth(bool IncludeCurrentDay = false)
	- DaysUntilTheEndOfTheYear(clsDate Date)
	- DaysUntilTheEndOfTheYear()
	- DayOrderInTheWeek(short Day, short Month, short Year)
	- DayOrderInTheWeek()
	- DaysFromYearStart(clsDate Date)
	- DaysFromYearStart()
	- GetDateFromDayOrderInTheYear(short DayOrderInYear, short Year)
	- DifferenceBetweenToDatesInDays(clsDate Date1, clsDate Date2, bool IncludEndDay = false)
	- DifferenceBetweenToDatesInDays(clsDate Date2, bool IncludEndDay = false)
	- Compare2Dates(clsDate Date1, clsDate Date2)
	- Compare2Dates(clsDate Date2)
	- CalculateMyAgeInDays(clsDate DateOfBirth)
	- PrintDate()
	- CalculateVacationDays(clsDate VacationStart, clsDate VacationEnd)
	- CalculateVacationDays(clsDate VacationEnd)
	- CalculateReturnDateFromVacation(clsDate VacationStart, short VacationDays)
	- CalculateReturnDateFromVacation(short VacationDays)
	- CalculateBusinessDays(clsDate DateFrom, clsDate DateTo)
	- PrintMonthCalendar(short Month, short Year)
	- PrintYearCalendar(short Year)
	- IncreaseDateByOneDay(clsDate Date)
	- IncreaseDateByXDays(int DaysToAdd, clsDate DateInfo)
	- IncreaseDateByOneWeek(clsDate Date)
	- IncreaseDateByXWeeks(int WeeksToAdd, clsDate Date)
	- IncreaseDateByOneMonth(clsDate Date)
	- IncreaseDateByXMonths(int MonthToAdd, clsDate Date)
	- IncreaseDateByOneYear(clsDate Date)
	- IncreaseDateByXYears(int YearsToAdd, clsDate Date)
	- IncreaseDateByOneDecade(clsDate Date)
	- IncreaseDateByXDecades(int DecadesToAdd, clsDate Date)
	- IncreaseDateByOneCentury(clsDate Date)
	- IncreaseDateByXCenturys(int CenturysToAdd, clsDate Date)
	- IncreaseDateByOneMillennium(clsDate Date)
	- IncreaseDateByXMillennium(int MillenniaToAdd, clsDate Date)
	- DecreaseDateByOneDay(clsDate Date)
	- DecreaseDateByXDays(int DaysToDecrease, clsDate DateInfo)
	- DecreaseDateByOneWeek(clsDate Date)
	- DecreaseDateByXWeeks(int WeeksToDecrease, clsDate Date)
	- DecreaseDateByOneMonth(clsDate Date)
	- DecreaseDateByXMonths(int MonthToDecrease, clsDate Date)
	- DecreaseDateByOneYear(clsDate Date)
...
public:
	...
	static bool IsValidDate(clsDate Date) {
		...
		if (Date._Day < 1 || Date._Day > MonthDays)
			return false;
	...
	static short CalculateMyAgeInDays(clsDate DateOfBirth)
	{
		...
		clsDate TodayDate = GetSystemDate(DateOfBirth);
